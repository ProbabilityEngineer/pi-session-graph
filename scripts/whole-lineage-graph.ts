...
function slug(path: string) {
	...
	if (!cwd) return path.split("/").at(-1)?.slice(0, 24) ?? "session";
	return cwd
...
function timestamp(path: string, fallback?: string) {
...
function nodeId(path: string) {
	let h = 0n;
	for (const ch of path) h = (h * 131n + BigInt(ch.charCodeAt(0))) % 9007199254740991n;
	return `n${h}`;
}
...
function edgeKind(record: any) {
	if (record.operationType === "repo_move") return "explicit-continuation";
	if (record.lineageKind) return record.lineageKind;
	if (record.edgeType) return record.edgeType;
	if (record.inferred) return "inferred";
	return "explicit-continuation";
}

function classFor(record: any) {
	const kind = edgeKind(record);
	if (String(kind).includes("new-lineage")) return "new";
	if (record.confidence === "low" || String(kind).includes("duplicate")) return "low";
	if (record.overlay || record.inferred || String(kind).includes("inferred")) return "inferred";
	return "explicit";
}
...
async function main() {
	...
	lines.push(
		...
		"  classDef explicit fill:#dcfce7,stroke:#16a34a;",
		"  classDef inferred fill:#fef9c3,stroke:#ca8a04;",
		"  classDef low fill:#fee2e2,stroke:#dc2626;",
		"  classDef manual fill:#ede9fe,stroke:#7c3aed;",
		"  classDef new fill:#f3f4f6,stroke:#6b7280;",
	);
