...
Implemented an initial local HTML viewer generated by `/session-graph html` and `pi-session-graph html`. It consumes the prepared graph model, supports search plus confidence/provider/edge-type filters, groups nodes by lane label, and shows node/edge detail JSON on selection. More advanced evidence panels, generic schemas, neighborhood export, and contradiction paths remain separate follow-up tickets.
