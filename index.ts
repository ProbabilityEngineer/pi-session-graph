...
function agentSessionStoreScriptCandidates(command: "build" | "build-store" | "export-graph" | "build-graphs") {
	const root = packageRoot();
	const script = ({ build: "build-curated-store", "build-store": "build-curated-store", "export-graph": "export-graph-json", "build-graphs": "build-graphs" } as const)[command];
	return [
		join(root, "node_modules", "agent-session-store", "dist", "scripts", `${script}.js`),
		join(root, "..", "node_modules", "agent-session-store", "dist", "scripts", `${script}.js`),
		join(root, "..", "agent-session-store", "dist", "scripts", `${script}.js`),
	].filter((value): value is string => Boolean(value));
}
...
async function runAgentSessionStore(command: "build" | "build-store" | "export-graph" | "build-graphs") {
	...
	for (const script of agentSessionStoreScriptCandidates(command)) {
		try {
			const { stdout, stderr } = await execFileAsync(process.execPath, [script], { env: process.env });
			return [stdout.trim(), stderr.trim()].filter(Boolean).join("\n");
		} catch (error) {
			const message = error instanceof Error ? error.message : String(error);
...
type ProjectMetricSummary = {
	...
	collapsedIntervals?: { start?: string; end?: string; activeMinutes?: number; activeHours?: number }[];
	status: "complete" | "partial" | "undercount-risk" | "overcount-risk";
	statusLabel: string;
	statusColor: string;
};
...
function projectMetricStatus(summary: Pick<ProjectMetricSummary, "coverageWarnings" | "excludedSessionIds" | "confidence">) {
	if (summary.excludedSessionIds.length) return { status: "overcount-risk", statusLabel: "overcount risk", statusColor: "#f97316" } as const;
	if (summary.coverageWarnings.length) return { status: "undercount-risk", statusLabel: "undercount risk", statusColor: "#facc15" } as const;
	if (summary.confidence === "low") return { status: "partial", statusLabel: "partial", statusColor: "#a78bfa" } as const;
	return { status: "complete", statusLabel: "complete", statusColor: "#22c55e" } as const;
}

const projectPalette = ["#60a5fa", "#f97316", "#22c55e", "#a78bfa", "#facc15", "#fb7185", "#2dd4bf", "#c084fc", "#38bdf8", "#f59e0b", "#84cc16", "#e879f9", "#14b8a6", "#818cf8", "#f43f5e", "#10b981"];
...
function summarizeProjectMetrics(graph: Graph) {
	...
	return [...metrics].map((metric) => {
		...
		const collapsedIntervals = Array.isArray(metadata?.collapsedIntervals) ? metadata.collapsedIntervals.filter((item): item is { start?: string; end?: string; activeMinutes?: number; activeHours?: number } => Boolean(item && typeof item === "object")) : undefined;
		const collapsedIntervalCount = collapsedIntervals?.length;
		...
		} satisfies ProjectMetricSummary;
...
function projectTimelineRows(graph: Graph) {
	const rows: { project: string; start: string; end: string; activeHours: number; provider: string; sessionId: string; contributingPath?: string; confidence?: string; status?: ProjectMetricSummary["status"]; statusLabel?: string; statusColor?: string; projectColor?: string; coverageWarnings?: string[]; excludedSessionIds?: string[]; rawIntervalCount?: number; collapsedIntervalCount?: number }[] = [];
...
function weeklyProjectRows(graph: Graph) {
	...
	for (const metric of summarizeProjectMetrics(graph)) {
		for (const interval of metric.collapsedIntervals ?? []) {
			...
			const hours = interval.activeHours ?? ((interval.activeMinutes ?? 0) / 60);
			const byWeek = totals.get(metric.displayName) ?? new Map<string, number>();
			byWeek.set(weekStart, (byWeek.get(weekStart) ?? 0) + hours);
			totals.set(metric.displayName, byWeek);
		}
	}
...
async function writeProjectVisualizations(reportDir: string, graph: Graph, stamp: string) {
	...
	await writeFile(ganttPath, `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(stamp)} — Project Gantt</title><script src="https://cdn.jsdelivr.net/npm/echarts@5/dist/echarts.min.js"></script><style>body{font-family:-apple-system,BlinkMacSystemFont,sans-serif;background:#111827;color:#e5e7eb;margin:0}header{padding:1rem;background:#0f172a;border-bottom:1px solid #334155}.muted{color:#94a3b8}.legend{display:flex;gap:12px;flex-wrap:wrap;margin-top:.5rem}.legend span{display:inline-flex;align-items:center;gap:6px}.swatch{width:12px;height:12px;border-radius:999px;display:inline-block}#chart{height:92vh}</style></head><body><header><h1>${escapeHtml(stamp)} — Project Gantt</h1><p class="muted">Canonical projects on rows, time on the x-axis, bar width by session span, color distinguishes projects, and tooltip includes confidence and exported warnings. Active-hour totals are timestamp-backed estimates, not guaranteed lifetime effort.</p><div class="legend"><span><i class="swatch" style="background:#60a5fa"></i>per-project colors</span><span class="muted">Coverage status remains in tooltips: complete, partial, undercount risk, overcount risk.</span></div></header><div id="chart"></div><script>const raw=${ganttData};const cats=[...new Set(raw.map(r=>r.project))].sort((a,b)=>raw.filter(x=>x.project===b).reduce((n,x)=>n+x.activeHours,0)-raw.filter(x=>x.project===a).reduce((n,x)=>n+x.activeHours,0));const el=document.getElementById('chart');el.style.height=Math.max(window.innerHeight*.92,cats.length*30+160)+'px';const chart=echarts.init(el);chart.setOption({backgroundColor:'#111827',tooltip:{formatter:p=>{const r=raw[p.dataIndex];return [r.project,r.start+' → '+r.end,'active: '+r.activeHours+'h','provider: '+r.provider,'status: '+(r.statusLabel||'unknown')+' ('+(r.confidence||'derived')+')',r.coverageWarnings&&r.coverageWarnings.length?'warnings: '+r.coverageWarnings.join(' | '):'',r.excludedSessionIds&&r.excludedSessionIds.length?'excluded sessions: '+r.excludedSessionIds.length:'',r.rawIntervalCount!=null?'raw intervals: '+r.rawIntervalCount:'',r.collapsedIntervalCount!=null?'collapsed intervals: '+r.collapsedIntervalCount:'',r.contributingPath||''].filter(Boolean).join('<br>')}},dataZoom:[{type:'slider',xAxisIndex:0,brushSelect:false,realtime:true},{type:'inside',xAxisIndex:0,zoomOnMouseWheel:false,moveOnMouseMove:true,moveOnMouseWheel:true},{type:'slider',yAxisIndex:0,brushSelect:false,realtime:true},{type:'inside',yAxisIndex:0,zoomOnMouseWheel:false,moveOnMouseMove:true,moveOnMouseWheel:true}],grid:{left:220,right:40,top:30,bottom:80},xAxis:{type:'time',axisLabel:{color:'#cbd5e1'}},yAxis:{type:'category',data:cats,axisLabel:{color:'#cbd5e1'}},series:[{type:'custom',renderItem:(params,api)=>{const cat=api.value(2);const start=api.coord([api.value(0),cat]);const end=api.coord([api.value(1),cat]);const h=Math.max(12, api.size([0,1])[1]*0.66);return {type:'rect',shape:{x:start[0],y:start[1]-h/2,width:Math.max(3,end[0]-start[0]),height:h},style:{fill:api.value(3)||'#60a5fa'}}},encode:{x:[0,1],y:2},data:raw.map(r=>[r.start,r.end,r.project,r.projectColor||'#60a5fa'])}]});window.addEventListener('resize',()=>chart.resize());</script></body></html>`);
	await writeFile(areaPath, `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(stamp)} — Weekly Project Area</title><script src="https://cdn.jsdelivr.net/npm/echarts@5/dist/echarts.min.js"></script><style>body{font-family:-apple-system,BlinkMacSystemFont,sans-serif;background:#111827;color:#e5e7eb;margin:0}header{padding:1rem;background:#0f172a;border-bottom:1px solid #334155}.muted{color:#94a3b8}.legend{display:flex;gap:12px;flex-wrap:wrap;margin-top:.5rem}.legend span{display:inline-flex;align-items:center;gap:6px}.swatch{width:12px;height:12px;border-radius:999px;display:inline-block}.legend-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:8px;margin-top:12px}.legend-item{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:6px 8px;border:1px solid #334155;border-radius:8px;background:#111827;opacity:.72;transition:opacity .12s ease,border-color .12s ease,background .12s ease}.legend-item.active{opacity:1;background:#172033;border-color:#93c5fd}.legend-item .left{display:flex;align-items:center;gap:8px;min-width:0}.legend-item .name{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.custom-tip{position:fixed;z-index:10;pointer-events:none;max-width:520px;background:rgba(255,255,255,.94);color:#0f172a;border:1px solid rgba(148,163,184,.55);border-radius:8px;padding:8px 10px;box-shadow:0 8px 24px rgba(0,0,0,.18);font-size:12px;line-height:1.35}.custom-tip.hidden{display:none}.tip-row{margin:2px 0;color:#334155}.tip-row.active{font-weight:800;color:#0f172a}#chart{height:92vh}</style></head><body><header><h1>${escapeHtml(stamp)} — Weekly Project Area</h1><p class="muted">Weekly available active hours by canonical project. Area color distinguishes projects; series labels and tooltips show confidence/coverage status. Totals are timestamp-backed estimates and can undercount or require overcount safeguards.</p><div class="legend"><span><i class="swatch" style="background:#60a5fa"></i>per-project colors</span><span class="muted">Coverage status remains in tooltips: complete, partial, undercount risk, overcount risk.</span></div><div id="legendList" class="legend-list"></div></header><div id="chart"></div><div id="customTip" class="custom-tip hidden"></div><script>const raw=${weeklyData};const weeks=[...new Set(raw.map(r=>r.week))].sort();const projects=[...new Set(raw.map(r=>r.project))].sort((a,b)=>raw.filter(x=>x.project===b).reduce((n,x)=>n+x.hours,0)-raw.filter(x=>x.project===a).reduce((n,x)=>n+x.hours,0)).slice(0,12);const meta=Object.fromEntries(raw.map(r=>[r.project,r]));const totals=Object.fromEntries(projects.map(name=>[name,raw.filter(r=>r.project===name).reduce((n,r)=>n+r.hours,0)]));const series=projects.map(name=>{const m=meta[name]||{};return {name:name+' ['+(m.statusLabel||'unknown')+']',type:'line',stack:'hours',smooth:true,lineStyle:{color:m.color||'#60a5fa',width:1.5},itemStyle:{color:m.color||'#60a5fa'},areaStyle:{opacity:.72},emphasis:{focus:'series',lineStyle:{width:3},areaStyle:{opacity:.72}},data:weeks.map(w=>{const row=raw.find(r=>r.project===name&&r.week===w);return row?row.hours:0})}});const chart=echarts.init(document.getElementById('chart'));let activeProject='';chart.setOption({backgroundColor:'#111827',color:${JSON.stringify(projectPalette)},tooltip:{show:false},legend:{show:false},grid:{left:60,right:40,top:60,bottom:80},xAxis:{type:'category',data:weeks,axisLabel:{color:'#cbd5e1',rotate:45}},yAxis:{type:'value',axisLabel:{color:'#cbd5e1',formatter:v=>v+'h'}},series});const legendList=document.getElementById('legendList');legendList.innerHTML=projects.map(name=>{const m=meta[name]||{};return '<div class="legend-item" data-project="'+name.replace(/"/g,'&quot;')+'"><div class="left"><i class="swatch" style="background:'+(m.color||'#60a5fa')+'"></i><span class="name">'+name+'</span></div><span>'+totals[name].toFixed(2)+'h</span></div>'}).join('');function setLegendActive(project){activeProject=project||'';for(const el of legendList.querySelectorAll('.legend-item')) el.classList.toggle('active', project && el.getAttribute('data-project')===project)}const customTip=document.getElementById('customTip');function showCustomTip(event,project){const point=chart.convertFromPixel({gridIndex:0},[event.offsetX,event.offsetY]);if(!point||point.length<2){customTip.classList.add('hidden');return}const weekIndex=Math.round(point[0]);if(weekIndex<0||weekIndex>=weeks.length){customTip.classList.add('hidden');return}const week=weeks[weekIndex];const rows=projects.map(name=>{const row=raw.find(r=>r.project===name&&r.week===week);return {name,hours:row?row.hours:0,marker:(meta[name]&&meta[name].color)||'#60a5fa'}}).filter(r=>r.hours>0);rows.sort((a,b)=>(a.name===project?0:1)-(b.name===project?0:1)||b.hours-a.hours);customTip.innerHTML='<strong>'+week+'</strong>'+rows.map(r=>'<div class=\"tip-row '+(r.name===project?'active':'')+'\"><span style=\"color:'+r.marker+'\">●</span> '+r.name+': '+r.hours.toFixed(2)+'h</div>').join('');customTip.style.left=Math.min(window.innerWidth-540,event.event.clientX+14)+'px';customTip.style.top=Math.min(window.innerHeight-220,event.event.clientY+14)+'px';customTip.classList.remove('hidden')}function projectAtPoint(offsetX,offsetY){const point=chart.convertFromPixel({gridIndex:0},[offsetX,offsetY]);if(!point||point.length<2)return '';const weekIndex=Math.round(point[0]);const y=point[1];if(weekIndex<0||weekIndex>=weeks.length||y==null)return '';const week=weeks[weekIndex];let cumulative=0;for(const name of projects){const row=raw.find(r=>r.project===name&&r.week===week);const hours=row?row.hours:0;const next=cumulative+hours;if(y>=cumulative&&y<=next)return name;cumulative=next;}return ''}chart.getZr().on('mousemove', event=>{const project=projectAtPoint(event.offsetX,event.offsetY);if(project){setLegendActive(project);showCustomTip(event,project)}else customTip.classList.add('hidden')});chart.on('mouseover',{seriesType:'line'},params=>{if(params.seriesName) setLegendActive(String(params.seriesName).replace(/ \[[^\]]+\]$/,''))});chart.on('highlight', params=>{if(params.seriesName) setLegendActive(String(params.seriesName).replace(/ \[[^\]]+\]$/,''))});chart.on('downplay', ()=>setLegendActive(''));chart.on('mouseout', ()=>setLegendActive(''));chart.on('globalout', ()=>{setLegendActive('');customTip.classList.add('hidden')});window.addEventListener('resize',()=>chart.resize());</script></body></html>`);
	await writeFile(treemapPath, `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(stamp)} — Project Treemap</title><script src="https://cdn.jsdelivr.net/npm/echarts@5/dist/echarts.min.js"></script><style>body{font-family:-apple-system,BlinkMacSystemFont,sans-serif;background:#111827;color:#e5e7eb;margin:0}header{padding:1rem;background:#0f172a;border-bottom:1px solid #334155}.muted{color:#94a3b8}.legend{display:flex;gap:12px;flex-wrap:wrap;margin-top:.5rem}.legend span{display:inline-flex;align-items:center;gap:6px}.swatch{width:12px;height:12px;border-radius:999px;display:inline-block}#chart{height:92vh}</style></head><body><header><h1>${escapeHtml(stamp)} — Project Treemap</h1><p class="muted">Overall allocation of available active hours. Tile color distinguishes projects; tooltips show confidence/coverage status. Size shows timestamp-backed effort, not guaranteed lifetime effort.</p><div class="legend"><span><i class="swatch" style="background:#60a5fa"></i>per-project colors</span><span class="muted">Coverage status remains in tooltips: complete, partial, undercount risk, overcount risk.</span></div></header><div id="chart"></div><script>const raw=${treemapData};const withContrast=raw.map((d,i)=>{const c=(d.color||${JSON.stringify(projectPalette)}[i%${projectPalette.length}]);const hex=c.replace('#','');const n=parseInt(hex,16);const r=(n>>16)&255,g=(n>>8)&255,b=n&255;const luminance=(0.2126*r+0.7152*g+0.0722*b)/255;const labelColor=luminance>0.68?'#0f172a':'#e5e7eb';return {...d,itemStyle:{color:c,borderColor:'#111827'},label:{color:labelColor}}});const chart=echarts.init(document.getElementById('chart'));chart.setOption({backgroundColor:'#111827',color:${JSON.stringify(projectPalette)},tooltip:{formatter:p=>{const d=p.data;return [d.name,'active: '+d.value+'h','status: '+(d.statusLabel||'unknown')+' ('+(d.confidence||'derived')+')','providers: '+(d.providers||[]).join(', '),d.coverageWarnings&&d.coverageWarnings.length?'warnings: '+d.coverageWarnings.join(' | '):'',d.excludedSessionIds&&d.excludedSessionIds.length?'excluded sessions: '+d.excludedSessionIds.length:'',d.rawIntervalCount!=null?'raw intervals: '+d.rawIntervalCount:'',d.collapsedIntervalCount!=null?'collapsed intervals: '+d.collapsedIntervalCount:'',((d.paths||[]).join('<br>'))].filter(Boolean).join('<br>')}},series:[{type:'treemap',roam:'move',breadcrumb:{show:false},label:{color:'#e5e7eb'},upperLabel:{show:true,color:'#e5e7eb'},itemStyle:{borderColor:'#111827'},data:withContrast}]});window.addEventListener('resize',()=>chart.resize());</script></body></html>`);
	return [
		...
		{ title: "Weekly Project Area", path: areaPath, description: "Weekly active-hour dominance by canonical project with per-project colors and coverage tooltips." },
		{ title: "Project Treemap", path: treemapPath, description: "Overall allocation of active hours by project with per-project colors and coverage tooltips." },
	] satisfies ReportArtifact[];
}
...
async function writeChartTimelineReports(reportDir: string, graph: Graph, stamp: string) {
	const make = async (path: string, title: string, group: "label" | "sessionId") => {
		...
		const data = spans.map((span) => {
			...
			const session = span.sessionId ?? span.id;
			const name = project;
			return { name, project, session, color: projectColor(project), value: [span.start, span.end ?? span.start, span.provider ?? "unknown", span.lineCount ?? 0, span.cwd ?? ""] };
		}).filter((row) => row.value[0]);
		const html = `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><script src="https://cdn.jsdelivr.net/npm/echarts@5/dist/echarts.min.js"></script><style>body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#111827;color:#e5e7eb;margin:0}#chart{height:92vh}header{padding:1rem;background:#0f172a;border-bottom:1px solid #334155}.muted{color:#94a3b8}</style></head><body><header><h1>${escapeHtml(title)}</h1><p class="muted">Chart-library timeline using Apache ECharts from CDN. Bars show session/activity spans; zoom and pan are enabled. No transcript content is included.</p></header><div id="chart"></div><script>const raw=${JSON.stringify(data).replace(/</g,"\\u003c")};const cats=[...new Set(raw.map(r=>r.name))].sort();const el=document.getElementById('chart');el.style.height=Math.max(window.innerHeight*.92,cats.length*30+160)+'px';const chart=echarts.init(el);chart.setOption({backgroundColor:'#111827',tooltip:{formatter:p=>{const r=raw[p.dataIndex];return r.project+'<br>'+r.value[0]+' → '+r.value[1]+'<br>session: '+r.session+'<br>provider: '+r.value[2]+'<br>lines: '+r.value[3]+'<br>'+r.value[4]}},dataZoom:[{type:'slider',xAxisIndex:0,brushSelect:false,realtime:true},{type:'inside',xAxisIndex:0,zoomOnMouseWheel:false,moveOnMouseMove:true,moveOnMouseWheel:true},{type:'slider',yAxisIndex:0,brushSelect:false,realtime:true},{type:'inside',yAxisIndex:0,zoomOnMouseWheel:false,moveOnMouseMove:true,moveOnMouseWheel:true}],grid:{left:260,right:40,top:30,bottom:80},xAxis:{type:'time',axisLabel:{color:'#cbd5e1'}},yAxis:{type:'category',data:cats,axisLabel:{color:'#cbd5e1',width:240,overflow:'truncate'}},series:[{type:'custom',renderItem:(params,api)=>{const cat=api.value(2);const start=api.coord([api.value(0),cat]);const end=api.coord([api.value(1),cat]);const h=Math.max(12, api.size([0,1])[1]*0.66);return {type:'rect',shape:{x:start[0],y:start[1]-h/2,width:Math.max(3,end[0]-start[0]),height:h},style:{fill:api.value(3)||'#60a5fa'}}},dimensions:['start','end','cat','color'],encode:{x:[0,1],y:2},data:raw.map(r=>[r.value[0],r.value[1],r.name,r.color||'#60a5fa'])}]});window.addEventListener('resize',()=>chart.resize());</script></body></html>`;
	...
	return [
		...
		{ title: "Chart timeline sessions", path: sessions, description: "Apache ECharts timeline grouped by repo/project with session details in tooltips." }
	] satisfies ReportArtifact[];
}
