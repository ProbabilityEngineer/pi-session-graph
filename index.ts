...
function dotGraph(graph: Graph, current?: string, options: { starts?: boolean } = {}) {
	...
	const branchCountByAgent = new Map<string, number>();
	for (const [source, records] of outgoingBySession) {
		...
		if (agent) branchCountByAgent.set(agent, (branchCountByAgent.get(agent) ?? 0) + fanout);
	}
	const laneScore = ([, nodes]: [string, SessionNode[]]) => Math.max(0, ...nodes.map((node) => branchCountByAgent.get(agentLabel(node) ?? propagatedAgent.get(node.path) ?? "") ?? 0));
	const sortedLanes = [...lanes.entries()].sort((a, b) => laneScore(b) - laneScore(a) || a[0].localeCompare(b[0]));
	const centeredLanes: Array<[string, SessionNode[]]> = [];
	for (const lane of sortedLanes) {
		if (centeredLanes.length % 2) centeredLanes.unshift(lane);
		else centeredLanes.push(lane);
	}
	let cluster = 0;
