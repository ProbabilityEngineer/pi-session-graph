...
- `npm run temporal-lineage` writes latest aliases `temporal-lineage.md/json/mmd/html`. Use `npm run temporal-lineage -- --snapshot` to also write timestamped snapshots under `snapshots/temporal-lineage/`. It models both topology and progression: purple circles are session starts discovered from JSONL filename timestamps, blue boxes are session-file topology nodes, yellow diamonds are time-indexed states of a source session, dotted arrows show progression inside the same append-only session file, and solid arrows show relocation/fork edges. To avoid Mermaid browser size limits, the diagram shows relocation-connected starts plus significant standalone starts by default (current lines ≥ 500, or up to 3 largest starts from cwd buckets with ≥ 5 sessions, excluding temp/test sessions); JSON includes all discovered starts. Use `--all-starts` only for raw/full graph experiments. `lines≤ts` is the accumulated JSONL row count in the source session at the relocation timestamp. It does not include transcript content.
