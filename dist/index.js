...
function agentSessionStoreScriptCandidates(command) {
    const root = packageRoot();
    const script = { build: "build-curated-store", "build-store": "build-curated-store", "export-graph": "export-graph-json", "build-graphs": "build-graphs" }[command];
    return [
...
function summarizeProjectMetrics(graph) {
    ...
    return [...metrics].map((metric) => {
        ...
        const collapsedIntervals = Array.isArray(metadata?.collapsedIntervals) ? metadata.collapsedIntervals.filter((item) => Boolean(item && typeof item === "object")) : undefined;
...
function weeklyProjectRows(graph) {
    ...
    for (const metric of summarizeProjectMetrics(graph)) {
        for (const interval of metric.collapsedIntervals ?? []) {
            ...
            const byWeek = totals.get(metric.displayName) ?? new Map();
